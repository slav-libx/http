...
var
  SSLLibHandle: HModule = 0;
  SSLUtilHandle: HModule = 0;
  SSLLibFile: string = '';
  SSLUtilFile: string = '';
...
type
  ...
  TSSLCipherDescription = function(_para1: SslPtr; buf: PAnsiChar; size: integer): PAnsiChar; cdecl;
...
var
  ...
  _SSLCipherDescription: TSSLCipherDescription = nil;
...
begin
  if InitSSLInterface and Assigned(_SSLGetCurrentCipher) then
    Result:=_SSLGetCurrentCipher(ssl);
end;

function SslCipherDescription(cssl: SslPtr): string;
var b:array[0..1024] of ansichar;
begin
  if InitSSLInterface and Assigned(_SslCipherDescription) then
    Result:=string(StrPas(_SslCipherDescription(cssl,@b[0],sizeof(b)-1)));
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        _SSLCipherDescription:=GetProcAddr(SSLLibHandle, 'SSL_CIPHER_description');
